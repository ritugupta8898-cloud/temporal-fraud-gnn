...
The jump from 0.6349 to 0.7350 represents the value of temporal memory — the model's ability to remember behavioral history across timesteps improves fraud detection on genuinely unseen future transactions by ~10 F1 points.

the loss while training the tgn model was significantly higher than the previous models because in the baseline the data of all timesteps was fed at once and the loss was calculated on that but here the loss is accumulated over all 34 timesteps so the total is naturally higher. the decrease from 23 to 1.4 over 50 epochs showed the model was learning.
...
- Elliptic Dataset — *Anti-Money Laundering in Bitcoin* — Elliptic, MIT